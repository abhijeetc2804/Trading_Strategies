...
void print1(vector<vector<string>>& vecOfVec) {
...
void print2(vector<vector<double>>& vecOfVec) {
   // cout<<vecOfVec.size()<<endl;
    for (const auto& innerVec : vecOfVec) {
        cout << "[ ";
        for (const auto& elem : innerVec) {
...
void write(vector<vector<double>>&data,vector<double>&coefficients,vector<string>&date_vector,int x,double p){  int position=0;
  int n=data[0].size();
  int i=n-1 ;
 ...
 // cout<<data[0][0]<<" "<<position<<endl;
  ...
  write_ordered_statistics(transactions,"ordered_statistics.csv");
  ofstream outputFile("final_pnl6.txt");
...
int main(int argc, char* argv[]) {
  ...
  //  cout<<train_data.size()<<endl;
   ...
   // print2(train_data);
    ...
    data=readCSV("LINEAR_REGRESSION_price_data.csv", {0,2,3,4,7,8,13});
