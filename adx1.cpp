...
vector<double> readPriceData(const string& filename){
  ...
  //  for (int i=-0;i<high.size();i++){cout<<dates[i]<<" "<<high[i]<<" "<<low[i]<<" "<<prices[i]<<"\n";}
    // cout<<endl;
    return prices;
}
...
int main(int argc, char* argv[]) {
    ...
    vector<double> prices = readPriceData("ADX_price_data.csv");
	// cout<<prices.size()<<" " <<dates.size()<<" @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"<<endl;
	// cout<<dates[0]<<" "<<dates[283]<<endl;
    // Run strategy
    ...
    ofstream finalPnLFile("final_pnl5.txt");
